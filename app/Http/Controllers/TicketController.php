...
class TicketController extends Controller
{
    ...
    public function create()
    {
        ...
        if (!Auth::user()->isQA() && !Auth::user()->isAdmin()) {
            abort(403, 'Hanya QA dan Admin yang dapat membuat tiket.');
        }
    ...
    public function store(StoreTicketRequest $request)
    {
        ...
        ActivityLog::create([
            ...
            'user_id' => Auth::id(),
            'action' => 'created',
            'description' => Auth::user()->name . " membuat tiket baru: {$ticket->ticket_number}",
        ]);

        return redirect()->route('tickets.show', $ticket)
            ->with('success', "Tiket {$ticket->ticket_number} berhasil dilaporkan!");
    }
    ...
    public function edit(Ticket $ticket)
    {
        ...
        if ($user->isQA() && $ticket->reporter_id !== $user->id) {
            abort(403, 'Anda tidak berhak mengedit tiket ini.');
        }
        ...
        if ($user->isDeveloper()) {
            ...
            if ($oldStatus !== $validated['status']) {
                ActivityLog::create([
                    ...
                    'description' => "{$user->name} memperbarui status tiket menjadi " . strtoupper($validated['status']),
            ...
            if ($oldStatus !== $ticket->status) {
                ActivityLog::create([
                    ...
                    'user_id' => $user->id,
                    'action' => 'status_changed',
                    'description' => "{$user->name} mengubah status tiket menjadi " . strtoupper($ticket->status),
            ...
            if ($oldPriority !== $ticket->priority) {
                ActivityLog::create([
                    ...
                    'description' => "{$user->name} mengubah prioritas tiket menjadi " . strtoupper($ticket->priority),
                    'changes' => ['old' => $oldPriority, 'new' => $ticket->priority],
                ]);
            }

            if ($oldAssigneeId != $ticket->assigned_to_id) {
                $newAssigneeName = $ticket->assignee ? $ticket->assignee->name : 'Belum Ditugaskan';
                ActivityLog::create([
                    'ticket_id' => $ticket->id,
                    'user_id' => $user->id,
                    'action' => 'assigned',
                    'description' => "{$user->name} menugaskan tiket ke: {$newAssigneeName}",
    ...
    public function destroy(Ticket $ticket)
    {
        ...
        if ($user->isAdmin() || ($user->isQA() && $ticket->reporter_id === $user->id && $ticket->status === 'open')) {
            ...
            ActivityLog::create([
                ...
                'user_id' => $user->id,
                'action' => 'deleted',
                'description' => "{$user->name} menghapus tiket: {$ticket->ticket_number} - {$ticket->title}",
            ]);
        ...
        abort(403, 'Anda tidak memiliki hak untuk menghapus tiket ini.');
    }
}
